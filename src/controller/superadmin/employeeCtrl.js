...
const EmployeeCtrl = {
    async  findAllEmployee(req, res) {
        ...
        // Sorting whitelist
        const allowedSortBy = new Set(['name', 'created_at', 'employee_level_id', 'department_id']);
        const sort_by = (req.query.sort_by ?? req.body?.sort_by ?? 'name').toString();
        ...
        const sortBy = allowedSortBy.has(sort_by) ? sort_by : 'name';
